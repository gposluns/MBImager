...
.O(okAA_O), // Buffer output
.IO(okAA), // Buffer inout port (connect directly to top-level port)
.I(okAA_I), // Buffer input
.T(okAA_T) // 3-state enable input, high=input, low=output
); 
