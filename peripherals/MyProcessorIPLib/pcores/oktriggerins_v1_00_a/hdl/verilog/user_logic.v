...
generate
	for (i = 0; i < 32; i = i + 1) begin
		okTriggerIn (.okHE(okHE), .ep_addr(0x40 + i), .ep_clk(Bus2IP_Clk), .ep_trigger(inputs[i*32 +: 32]));
		assign reg_i[i] = |inputs[i*32 +: 32];
	end
endgenerate;
	assign Interrupt = |reg_i;
