...
input		[7:0] 						  DATA_IN_FROM_PINS3;
output		[15:0] 						  DATA_IN_TO_DEVICE3;
input 									  CLK_IN;
input 									  CLK_EN;
...
integer byte_index;
  ...
  generate for (pin_count = 0; pin_count < 8; pin_count = pin_count + 1) begin: pins
    ...
    IBUF
      #(.IOSTANDARD ("LVCMOS33"))
     ibuf_inst3
       (.I          (DATA_IN_FROM_PINS3    [pin_count]),
        .O          (data_in_from_pins_int3[pin_count]));
	...
	assign data_in_from_pins_delay3[pin_count] = data_in_from_pins_int3[pin_count];
   ...
   // iodelay2_inst3
    ...
    IDDR2
     #(.DDR_ALIGNMENT  ("C0"),
       ...
       .SRTYPE         ("ASYNC"))
     iddr2_inst3
      (.Q0             (DATA_IN_TO_DEVICE3[pin_count]),
       .Q1             (DATA_IN_TO_DEVICE3[8 + pin_count]),
       .C0             (clk_in_inv),
       .C1             (CLK_IN),
       .CE             (CLK_EN),
       .D              (data_in_from_pins_delay3[pin_count]),
       .R              (1'b0),
       .S              (1'b0));
  end
  endgenerate
